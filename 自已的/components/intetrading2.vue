<template>
      ...
      <ul>
        <li v-for="(item,i) in strategy_list" :key="i" style="position: relative;">
          <p class="cometop" >
			...
			<span style="font-size: .22rem;margin-left: 17px;"><span class="item-span" :class="{'item-span1':item.zhangfu>0}" style="font-size: .24rem;"> {{item.close1?item.close1:''}}</span> </span>
			<span style="position: absolute;right: 25px;">
				<button
				 ...
				 >删除策略</button>
             <button
                style="background:rgb(97,161,240);"
                v-show="item.types==2"
                @click="fn6(item)"
              >设置策略</button>
              <button
 ...
 <li class="li-item"  v-for="(item,i) in list3"
	...
	<van-popup v-model="show3" position="top" :style="{ height: '100%'}" >
		<p class="popup2" style="height: 40px;line-height: 40px;font-weight: 550;font-size: 18px;text-align: center;color: rgb(80,80,80);"><van-icon name="arrow-left" @click="fn4()"/> 策略设置</p>
	<div class="divli1">
	...
	<button type="button" class="changebton"  style="background-color: #2167ff;margin: .7rem auto .3rem;width: 71%;height: .6rem;line-height: .6rem;" @click="fn4(true)">应用设置</button>
	</van-popup>
  </div>
</template>
...
export default {
  ...
  methods: {
	  ...
	  fn4(bool=false){
		  if(bool){
			  ...
			  if(this.show5 && this.backinfo.loss_stop == 0){
				return this.$toast.fail({ message: "止损比例不能设置为零", duration: 3200 });
			  }
			  let str;
			  this.istxt = 100;
	  ...
	  fn5(id,type){
		  ...
		  Dialog.confirm({
		    ...
		    message: '是否删除策略？',
		  		...
		  		this.$axios
		  		.post("/index/spot/delete_strategy",{'strategy_id':id})
		  		.then(res => {
		  				if(res.data.code == 0){
	  ...
	  fn6(item){
		  if(item.stop_type == 2){
		  			  this.$toast.fail({ message: "止盈停止后才可设置", duration: 2000 })
		  			  return
		  }
		  this.bool2 = true
		  this.backinfo = item
	  ...
	  tocaozuo(){
		  ...
		  .then(res => {
		   this.$router.push({path:'noticedateil',query:{id:res.data.info[1].article_id}})
		  });
	  },
	   onInput(i,id) {
		   this.index1 = i
    ...
    selecli_symbolcli(i,index) {
		console.log(i,index,this.symbol_list)
    ...
    open_strategy(s,i) {
      this.$axios.post("/index/spot/start_strategy", { id: s }).then(res => {
        if (res.data.code == 0) {
    ...
    close_now() {
      if (this.check == 0) {
        ...
        // var id = s;
        this.$axios
          .post("/index/spot/pause_strategy", { id: this.closeid })
          .then(res => {
            if (res.data.code == 0) {
              this.$toast.success({ message: res.data.msg, duration: 2000 });
		  ...
		  this.$axios
		      .post("/index/spot/profit_stop", { id: this.closeid })
		      .then(res => {
		        if (res.data.code == 0) {
		          this.$toast.success({ message: res.data.msg, duration: 2000 });
		          this.start();
