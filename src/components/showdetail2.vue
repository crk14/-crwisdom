<template>
	<div class="inpareport" style="min-height: 500px;">
		...
		<van-pull-refresh v-model="isLoading" success-text="刷新成功" @refresh="onRefresh">
			...
			<div>
				<div v-show="type == 'bibi'" style="display: flex;width: 90%;margin: 2% 5%;color: #99A1A7;border: 1px solid #f6f7f9;font-size: 0.28rem;">
				...
				<div style="display: flex; padding-left:.2rem;font-size: 12px;margin-top: .4rem;margin-bottom: .2rem">{{starttime}}至
					{{endtime}}
				...
				<ul class="page-ui" v-show="bool1==0 && folshow">
					<li v-if="type == '2' ||type == 'fol' " v-for="(item,i) in list" :key="i">
						<div class="title">
							<div class="left">
					...
					<li v-if="type == 'bibi'" v-for="(item,i) in list" :key="i">
						...
						<div class="body" id="body-s">
							<div>数量{{'('+item.symbol1.toUpperCase()+')'}}
								<p>{{item.cjsl}}</p>
							</div>
							<div>成交均价({{symbol.toUpperCase()}}) <p>{{(item.field_cash_amount /item.cjsl).toFixed(4)}}</p></div>
							<div style="text-align: right;">成交总额({{symbol.toUpperCase()}})<p style="right: 0;">{{item.field_cash_amount}}</p></div>
						</div>
						<div class="body" style="margin-top: .15rem;" id="body-s">
							<div>手续费(USDT) <p>{{ poni?0:item.cjsy>0?(item.cjsy * 0.3).toFixed(4):0}}</p></div>
							<div>利润({{symbol.toUpperCase()}}) <p>{{Number(item.cjsy).toFixed(4) }}</p></div>
							<div style="text-align: right;">利润率(%)<p style="right: 0;">{{item.syl}}</p></div>
						</div>
					</li>
				</ul>
	...
	export default {
		...
		methods: {
			...
			jiesuan() {
				...
				let str = `${img}/${trant}transaction_list`
				if (this.type == 'bibi') {
					str = `spotfollow/transaction_list`
				}
				this.$axios.post(`/index/${str}`, {
					symbol: this.symbol,
					bourse: this.bourse,
